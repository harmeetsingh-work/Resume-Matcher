...
| [custom-prompts.md](docs/agent/70-features/custom-prompts.md)                                       | AI prompt customization & section regeneration     |
...
- `app/prompt_registry.py` - Custom prompt management (load, save, enable/disable)
- `app/routers/` - API endpoints (health, config, resumes, jobs, prompts)
- `app/services/` - Business logic (parser, improver, section_regenerator)
- `app/schemas/` - Pydantic models matching frontend contracts
- `app/prompts/` - LLM prompt templates (templates, enrichment, sections)
...
- **Prompt Registry**: All prompts are managed via `app/prompt_registry.py`. Use `get_prompt("prompt_id")` to retrieve prompts in services—this enables user customization.
- **Prompt Guidelines**:
  ...
  - Register new prompts in `PROMPT_REGISTRY` with metadata (name, description, category, variables, used_in)
...
| Category                 | Prompts                                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------- |
| **Parsing & Matching**   | `parse_resume`, `extract_keywords`                                                       |
| **Enhancement**          | `improve_resume`, `analyze_resume`, `enhance_description`                                |
| **Outreach**             | `cover_letter`, `outreach_message`                                                       |
| **Section Regeneration** | `regenerate_summary`, `regenerate_experience`, `regenerate_project`, `regenerate_skills` |

### Prompt Features

- **View & Edit**: Collapsible editors with syntax highlighting
- **Token Count**: Approximate token count displayed for each prompt
...
| File                                                  | Purpose                      |
| ----------------------------------------------------- | ---------------------------- |
| `apps/backend/app/prompt_registry.py`                 | Central prompt management    |
| `apps/backend/app/routers/prompts.py`                 | Prompts API endpoints        |
| `apps/backend/app/prompts/templates.py`               | Default prompt content       |
| `apps/backend/app/prompts/sections.py`                | Section regeneration prompts |
| `apps/frontend/lib/api/prompts.ts`                    | Prompts API client           |
| `apps/frontend/components/settings/prompt-editor.tsx` | Prompt editor component      |
...
- Tone preferences (professional, confident, friendly)

**Endpoint**: `POST /api/v1/resumes/{resume_id}/regenerate-section`

### Storage

Custom prompts stored in `apps/backend/data/prompts.json` (gitignored).
